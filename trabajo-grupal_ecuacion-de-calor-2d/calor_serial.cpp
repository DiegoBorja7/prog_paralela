...
{
    u.assign(params.nx * params.ny, 0.0);

    for (int i = 0; i < params.nx; i++)
    {
        u[0 * params.nx + i] = 100.0;
    }
}
...
double calcular_calor_serial(const ParametrosCalor &params, const std::vector<double> &u_old, std::vector<double> &u_new)
{
    ...
    // Copiamos los bordes fijos
    for (int i = 0; i < params.nx; i++)
    {
        u_new[0 * params.nx + i] = u_old[0 * params.nx + i];                             // Borde superior
        u_new[(params.ny - 1) * params.nx + i] = u_old[(params.ny - 1) * params.nx + i]; // Borde inferior
    }
    for (int j = 0; j < params.ny; j++)
    {
        u_new[j * params.nx + 0] = u_old[j * params.nx + 0];                             // Borde izquierdo
        u_new[j * params.nx + (params.nx - 1)] = u_old[j * params.nx + (params.nx - 1)]; // Borde derecho
    }

    // Esquema de 5 puntos (Jacobi) para el interior
